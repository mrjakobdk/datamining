...
void RTree::insert(RTree_Node *u, std::pair<int, float *> *p) {
    if (u->is_leaf()) {
...
void RTree::insert(RTree_Node *E) {//todo not sure this is correct
...
RTree_Node *RTree::choose_subtree(RTree_Node *u, RTree_Node *E) {
    RTree_Node *best_v;
...
RTree_Node *RTree::choose_subtree(RTree_Node *u, std::pair<int, float *> *p) {
    RTree_Node *best_v;
    float best_inc_perimeter = std::numeric_limits<float>::max();
    for (RTree_Node *v: u->children) {
        float inc_perimeter = 0.;
...
void RTree::handle_overflow(RTree_Node *u) {
...
RTree_Node *RTree::split_leaf(RTree_Node *u) {
    int m = u->points.size();
...
RTree_Node *RTree::split_internal(RTree_Node *u) {
...
float RTree::dist(float *p, float *q) {
    float sum = 0.;
    for (int j = 0; j < this->dims; j++) {
        float sub = p[j] - q[j];
        sum += sub * sub;
...
void RTree::range(std::vector<int> &R, RTree_Node *u, float eps, float *p) {
    if (u->is_leaf()) {
...
RTree_Node *RTree::node_query(RTree_Node *u, std::pair<int, float *> *p) {
    if (u->is_leaf()) {
        if (u->contains(p)) {
            return u;
        }
...
RTree_Node *RTree::node_query_check_all(RTree_Node *u, std::pair<int, float *> *p) {
    if (u->is_leaf()) {
        if (u->contains(p)) {
            return u;
        }
...
//L is a leaf node
void RTree::condense_tree(RTree_Node *L) {
    int m = 0.4 * B;
...
void RTree::insert(std::pair<int, float *> *p) {
    this->insert(root, p);
}
...
void RTree::remove(std::pair<int, float *> *p) {
    RTree_Node *L;
    if (root->is_leaf()) {
...
std::vector<int> RTree::range(float eps, float *p) {
    std::vector<int> R;
